...
class CustomFilters
{
    ...
    public function getCustomFilters($tableId, $tableVars)
    {
        ...
        foreach ($filters as $filterIndex => $filter) {
            if ($filter['type'] == 'date_picker' || $filter['type'] == 'date_range') {
                if (isset($filter['columns'][0]) && isset($formattedColumns[$filter['columns'][0]]['formatString'])) {
    ...
    public function addAdvancedFilter($tableVars, $tableId)
    {
        ...
        wp_enqueue_script('ninja-tables-pro', NINJAPROPLUGIN_URL . 'assets/ninja-tables-pro.js', array('jquery'), NINJAPROPLUGIN_VERSION, true);
        $this->addFilterJS($table_instance_name, $customFilters, $tableId);
    ...
    private function addFilterJS($filter_key, $filters, $tableId)
    {
        ob_start();
        ?>
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                ...
                var FilterOptions = ninjaTableGetCustomFilter(filters, filterKey, tableId);
